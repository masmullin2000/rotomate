...
# Run the project
run *ARGS:
    cargo run --target x86_64-unknown-linux-musl {{ARGS}}
...
test-run:
    just run -- config/single.yaml -v --root

# Run with split config files
test-run-split:
    just run -- config/hosts.yaml config/runs.yaml config/tasks.yaml -v --root

# Run with config that uses imports
test-run-imports:
    just run -- config/main.yaml -v
...
test-upload count="100": (build "release")
    ...
    # cargo build -r
    time ./target/x86_64-unknown-linux-musl/release/rot config/upload_test.yaml
...
test-upload-debug-build count="100":
    ...
    RUST_LOG=debug cargo run -- config/upload_test.yaml 2>&1
